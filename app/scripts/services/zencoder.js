...
angular.module('video-ads')
  .service('Zencoder', function Zencoder($http, $q, $modal) {
    ...
    this.onVideoFileUpload = function (additionalNewVideoUploadParams){
      ...
      fileInput.bind('change', function () {
        ...
        getNewVideoUploadCredentials(file, additionalNewVideoUploadParams)
          .then(uploadToS3)
          .then(encode, angular.noop, function (uploadPercentComplete) {
            clickDeferred.notify(uploadPercentComplete);
          })
          .then(
    ...
    function getNewVideoUploadCredentials(file, additionalNewVideoUploadParams) {
      var data = {
        name: file.name
      };

      _.extend(data, additionalNewVideoUploadParams);

      var newVideoDeferred = $q.defer();

      $http.post(newVideoUrl, data)
